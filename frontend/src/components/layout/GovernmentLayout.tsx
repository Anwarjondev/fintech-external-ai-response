...
type GovernmentLayoutProps = { children: ReactNode };
...
export function GovernmentLayout({ children }: GovernmentLayoutProps) {
  ...
  const displayName = user?.full_name ?? user?.fullName ?? user?.phone ?? "User";
  const org = user?.organization ?? "Government";
    ...
    <div className="flex h-screen bg-app">
      ...
      <aside className="w-64 flex flex-col bg-elevated border-r border-line">
        <div className="p-6 border-b border-line">
          ...
          <p className="text-xs text-subtle mt-1">{String(org)}</p>
        </div>
